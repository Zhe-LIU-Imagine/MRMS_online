...
void centerMatches(const Image<float>& I1,const Image<float>& I2,
	std::vector<keypoint>& F1, std::vector<keypoint>& F2,
	std::vector<Pair>& matches, libNumerics::matrix<float>& E,std::vector<bool>& valide,KvldParameters& kvldParameters,
	std::vector<Pair>& output){
		std::cout<<"If we take only inner points "<<std::endl;
		output.clear();
		float range1=getRange(I1,std::min(F1.size(),matches.size()),kvldParameters.inlierRate);
		float range2=getRange(I2,std::min(F2.size(),matches.size()),kvldParameters.inlierRate);
		...
		//We are selecting matches, so it would be rigorous to test both images the angle,but not necessary
		float openAng= PI*5/6;
		for (int it1=0; it1<size;it1++){
			if (valide[it1]){
				size_t a1=matches[it1].first, b1=matches[it1].second;
				float minAng=0; 
				...
				for (int it2=0; it2<size;it2++){
					if (it1!=it2 && valide[it2]){
						...
						if ( dist1>min_dist && dist2>min_dist && (dist1<range1 || dist2<range2) && E(it1,it2)>=0){
							float angle;
							anglefrom(F1[a2].x-F1[a1].x, F1[a2].y-F1[a1].y, angle);
							
							if (first){ ref=angle; first=false;}
							...
							if (angle>0){
								if(angle<openAng) pos=true;
								if(angle>maxAng) maxAng=angle;
							}else{
								if(angle>-openAng) neg=true;
								if(angle<minAng) minAng=angle;
							}
							if (pos&&neg&& (maxAng-minAng)>2*PI-openAng){
								output.push_back(matches[it1]);
								break;
							}
						}
					}
