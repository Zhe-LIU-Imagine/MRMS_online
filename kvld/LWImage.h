...
public:
	...
	//inline T  get(float x0, float y0) const{//bilineare
	//	float x=x0-0.5,y=y0-0.5;
	//	int ix=int(x), iy=int(y);
	//	float dx=x-ix, dy=y-iy;
	//	if (dx==0 && dy==0)
	//		return data[ w* int(y) + int(x)];
	//	if (dx==0)
	//		return data[ w * iy + ix] *(1-dy)+ data[w * (iy+1) + ix]*dy;
	//	if (dy==0)
	//		return  data[ w * iy + ix]*(1-dx)+ data[ w * iy + ix+1]*dx;
	//	return  
	//		data[ w *  iy    + ix  ]*(1-dx)*(1-dy)+
	//		data[ w * (iy+1) + ix  ]*dy*(1-dx)+
	//		data[ w *  iy    + ix+1]*(1-dy)*dx+
	//		data[ w * (iy+1) + ix+1]*dx*dy;
	//}
