...
public:
  ...
  inline T  get(float y, float x) const{
 
    int ix=int(x), iy=int(y);
		float dx=x-ix, dy=y-iy;
		if (dx==0 && dy==0)
			return _data[ _width * int(y) + int(x)];
		if (dx==0)
			return _data[ _width * iy + ix] *(1-dy)+ _data[ _width * (iy+1) + ix]*dy;
		if (dy==0)
			return  _data[ _width * iy + ix]*(1-dx)+ _data[ _width * iy + ix+1]*dx;

		return  
      _data[ _width *  iy    + ix  ]*(1-dx)*(1-dy)+
			_data[ _width * (iy+1) + ix  ]*dy*(1-dx)+
			_data[ _width *  iy    + ix+1]*(1-dy)*dx+
			_data[ _width * (iy+1) + ix+1]*dx*dy;
  }
