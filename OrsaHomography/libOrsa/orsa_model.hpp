...
class OrsaModel {
 public:
  ...
  virtual double Error(const Model &model, size_t index, int* side=0) const = 0;
  ...
  virtual void Fit(const std::vector<size_t> &indices,
                   std::vector<Model> *models,std::vector<float> & weight=std::vector<float>(), int OPmethod=0) const = 0;
  ...
  double orsa(std::vector<size_t> &vec_inliers,
              ...
              Model *model = NULL,
              bool bVerbose = false,bool LocOpt=false) const;
double ransac(std::vector<size_t> & vec_inliers,
                       size_t nIter,
                       double *precision,
                       Model *model,bool LocOpt=false)const ;
double mlesac(std::vector<size_t> & vec_inliers,
                       size_t nIter,
                       double *precision,
                       Model *model)const ;
double prosac(std::vector<size_t> &vec_inliers,
              size_t nIter = 10000,
              double *precision=NULL,
              Model *model = NULL,
              bool LocOpt = false) const;
  /// Compute model from points.
  bool ComputeModel(const std::vector<size_t> &indices,
                    Model *model, std::vector<float>& weight=std::vector<float>(),int OPmethod=0) const;
...
private:
  ...
  struct ErrorIndex {
    ...
    size_t index; ///< Correspondence index
    int side;     ///< Error in image 1 (side=0) or 2 (side=1)?
    /// Constructor
    ErrorIndex(double e=0, size_t i=0, int s=0): error(e), index(i), side(s) {}
